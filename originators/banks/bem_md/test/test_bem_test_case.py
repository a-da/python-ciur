...
class BEMTestCase(unittest.TestCase):
    def runTest(self):
        ...
        dpf = DomParserFile(
            name = "test",
            source = "../xjsons/banca_de_economii.json"
        )
        for i in [1]:
            expect = AdvancedDict()
            expect.load_json("./expect/banca_de_economii_%02d.json" %i)
            expect = AdvancedDict.pretty_float(expect)

            xpath = dpf.get_version()
            dpf.validate_configs(xpath)
            with open("../page_samples/banca_de_economii_%02d.html" %i) as f: html = f.read()
