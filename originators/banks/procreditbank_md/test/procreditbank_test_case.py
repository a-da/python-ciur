...
class ProcreditbankTestCase(unittest.TestCase):
    def runTest(self):
        os.chdir(os.path.dirname(__file__))

        dpf = DomParserFile(
            name   = "test",
            source = "../xjsons/procreditbank.json"
        )
        for i in [1]:
            expect = AdvancedDict()
            expect.load_json("./expect/procreditbank_%02d.json" %i)
            expect = AdvancedDict.pretty_float(expect)

            xpath = dpf.get_version()
            dpf.validate_configs(xpath)
            with open("../page_samples/procreditbank_%02d.html" %i) as f: html = f.read()
