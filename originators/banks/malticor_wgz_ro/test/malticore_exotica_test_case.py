...
class MalticoreExoticaTestCase(unittest.TestCase):
    def runTest(self):
        os.chdir(os.path.dirname(__file__))

        dpf = DomParserFile(
            name   = "test",
            source = "../xjsons/malticor_exotica.json"
        )
        for i in [1]:
            expect = AdvancedDict()
            expect.load_json("./expect/malticor_exotica_%02d.json" %i)
            expect = AdvancedDict.pretty_float(expect)

            xpath = dpf.get_version()
            dpf.validate_configs(xpath)
            with open("../page_samples/malticor_exotica_%02d.html" %i) as f: html = f.read()
